...
sub delete_ior_files {
    ...
    $tg_executor->DeleteFile ($nm_app_ior);
    $tg_executor->DeleteFile ($nm_am_ior);
}
...
print "Start dancex11_plan_launcher.exe with -l $plan_file -r file://".$tg_executor->LocalFile($iorbases[0])." --app-ior $nm_app_ior --am-ior $nm_am_ior\n";
$E = $tg_executor->CreateProcess ("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_plan_launcher",
                        ...
                        " --app-ior " . $tg_executor->LocalFile($nm_app_ior) .
                        " --am-ior " . $tg_executor->LocalFile($nm_am_ior));
...
if ($tg_executor->WaitForFileTimed($nm_app_ior,
                                   $tg_executor->ProcessStartWaitInterval ()) == -1) {
    print STDERR
        "ERROR: The ior $nm_app_ior file of the NM Application could not be found\n";
    kill_open_processes ();
    exit 1;
}
...
print "by running dancex11_plan_launcher with -x $plan_file -r file://".$tg_executor->LocalFile($iorbases[0])." --app-ior file://$nm_app_ior --am-ior file://$nm_am_ior\n";

$E = $tg_executor->CreateProcess ("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_plan_launcher",
                        "-x " . $tg_executor->LocalFile($plan_file) . " -r file://".$tg_executor->LocalFile($iorbases[0]) .
                        " --app-ior file://" . $tg_executor->LocalFile($nm_app_ior) . " --am-ior file://" . $tg_executor->LocalFile($nm_am_ior));
$pl_status = $E->SpawnWaitKill ($PerlACE::Process::WAIT_DELAY_FACTOR * $tg_executor->ProcessStartWaitInterval ());
