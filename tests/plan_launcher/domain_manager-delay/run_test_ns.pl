...
print "Start dancex11_plan_launcher.exe with -l $plan_file -n ExecutionManager --deployment-nc " . $ENV{'NameServiceIOR'} . "\n";
$E = $tg_executor->CreateProcess ("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_plan_launcher",
                        "-l " . $tg_executor->LocalFile($plan_file) . " -t 10  -n ExecutionManager --deployment-nc " . $ENV{'NameServiceIOR'});
...
print "Invoking domain manager (dancex11_deployment_manager --handler dancex11_domain_dm_handler) with -n ExecutionManager --deployment-nc " . $ENV{'NameServiceIOR'} ."\n";
...
print "by running dancex11_plan_launcher with -x $plan_file -n ExecutionManager --deployment-nc " . $ENV{'NameServiceIOR'} . "\n";

$E = $tg_executor->CreateProcess ("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_plan_launcher",
                        "-x " . $tg_executor->LocalFile($plan_file) . " -n ExecutionManager --deployment-nc " . $ENV{'NameServiceIOR'});
