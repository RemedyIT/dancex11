...
print "Start dancex11_plan_launcher.exe with -l $plan_file -r file://".$tg_executor->LocalFile($iorbases[0])." --app-ior $lm_app_ior --am-ior $lm_am_ior\n";
...
print "by running dancex11_plan_launcher with -x $plan_file -r file://".$tg_executor->LocalFile($iorbases[0])." --app-ior file://$lm_app_ior --am-ior file://$lm_am_ior\n";

$E = $tg_executor->CreateProcess ("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_plan_launcher",
                        "-x " . $tg_executor->LocalFile($plan_file) . " -r file://" . $tg_executor->LocalFile($iorbases[0]) .
                        " --app-ior file://" . $tg_executor->LocalFile($lm_app_ior) . " --am-ior file://" . $tg_executor->LocalFile($lm_am_ior));
$pl_status = $E->SpawnWaitKill ($PerlACE::Process::WAIT_DELAY_FACTOR * $tg_executor->ProcessStartWaitInterval ());
