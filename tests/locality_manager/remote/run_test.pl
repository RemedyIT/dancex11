...
sub run_node_daemon {
    ...
    $lm_ior = $tg_daemon->LocalFile ('inst_one.ior');
    $d_param = "--handler dancex11_locality_dm_handler -a localhost -N -n LocalityInstance=$lm_ior";

    print "Run $d_cmd with $d_param\n";

    $LM_DEAMON = $tg_daemon->CreateProcess ($d_cmd, $d_param);
    $LM_DEAMON->Spawn ();

    if ($tg_daemon->WaitForFileTimed('inst_one.ior',
                                     $tg_daemon->ProcessStartWaitInterval ()) == -1) {
        print STDERR
            "ERROR: The ior $iorfile file of node daemon $i could not be found\n";
        $LM_DEAMON->Kill (); $LM_DEAMON->TimedWait (1);
        return -1;
    }
...
# Run an external script on the target which checks on which cpu the dancex11_deployment_manager
# is running.
$CHECKER = $tg_daemon->CreateProcess ("check_lm.sh");
$outcome = $CHECKER->SpawnWaitKill ($tg_daemon->ProcessStartWaitInterval ());
if ($outcome == 1) {
