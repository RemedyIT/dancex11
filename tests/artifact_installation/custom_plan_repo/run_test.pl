...
sub run_node_daemon {
    ...
    $config_file = $tg_daemon->LocalFile ('../nodemanager.config');

    $d_cmd = "$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_deployment_manager";
    $d_param = "--handler dancex11_node_dm_handler -c $config_file -p $port -N -n $nodename=$iorfile --deployment-nc ". $ENV{'NameServiceIOR'};
...
# is running.
$CHECKER = $tg_daemon->CreateProcess ("check_artifacts.sh");
$outcome = $CHECKER->SpawnWaitKill ($tg_daemon->ProcessStartWaitInterval ());
if ($outcome == 1) {
    print STDERR "ERROR: Artifacts checker returned $outcome\n";
}
