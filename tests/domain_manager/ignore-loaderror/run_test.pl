...
$config_file = $tg_exe_man->LocalFile ('./domainmanager.invalid-cdp.config');
print "Invoking domain deployment manager (dancex11_deployment_manager --handler dancex11_domain_dm_handler -c $config_file) with -l $plan_file\n";
$EM = $tg_exe_man->CreateProcess ("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_deployment_manager",
                                    "--handler dancex11_domain_dm_handler -c $config_file -l " . $tg_exe_man->LocalFile ($plan_file) .
                                    " -n ExecutionManager=$ior_emfile -- " .
                                    "--node ". $nodenames[0] . "=file:///" . $iorfiles[0] . " " .
                                    "--node ". $nodenames[1] . "=file:///" . $iorfiles[1]);
$em_status = $EM->Spawn ();
...
if ($tg_exe_man->WaitForFileTimed ($ior_embase,
    ...
    if ($plan_fmt eq 'config') {
      print STDERR
        "ERROR: The ior file of execution manager could not be found\n";
      kill_open_processes ();
      exit 1;
    } else {
      print "NOTE: dancex11_deployment_manager correctly failed to launch $plan_file\n";
      kill_open_processes ();
      exit 0;
    }
}
