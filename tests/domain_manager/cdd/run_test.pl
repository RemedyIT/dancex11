...
$plan_fmt = "config";
$cdd_fmt = "cfg";
# Are we testing the convert plan support?
$convert_plan = 0;
$conversion_fmt = 'ddcfg';
...
$nodemap_base = "nodemap";
$nodemap = $nodemap_base . "." . $cdd_fmt;
...
sub create_targets {
    ...
    #   convert plan
    $tg_convert_plan = PerlACE::TestTarget::create_target (1) || die "Create target for convert plan failed\n";
}
...
sub delete_ior_files {
    ...
    $tg_convert_plan->DeleteFile ($plan_base . ".cdr");
    $tg_convert_plan->DeleteFile ($nodemap_base . ".cdr");
}
...
sub run_node_daemons {
    for ($i = 0; $i < $nr_daemon; ++$i) {
        ...
        $d_param = "--handler dancex11_node_dm_handler -p $port -N -n $nodename=$iorfile";
...
if ($convert_plan eq 1) {
  print "Converting plan $plan_file to CDR representation\n";
  $cdr_outputfile = $plan_base . ".cdr";
  $convert = $tg_convert_plan->CreateProcess("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_convert_plan",
                                               "-i " . $tg_convert_plan->LocalFile ($plan_file) .
                                               " -o " . $tg_convert_plan->LocalFile ($cdr_outputfile));

  $convert_status = $convert->SpawnWaitKill ($PerlACE::Process::WAIT_DELAY_FACTOR * $tg_convert_plan->ProcessStartWaitInterval ());

  if ($convert_status != 0) {
      print STDERR "ERROR: dancex11_convert_plan returned $convert_status";
      kill_open_processes ();
      exit 1;
  }

  if ($tg_convert_plan->WaitForFileTimed ($cdr_planbase, $tg_convert_plan->ProcessStartWaitInterval ()) == -1) {
    print STDERR "ERROR: dancex11_convert_plan failed to convert $plan_file to $cdr_outputfile.\n";
    kill_open_processes ();
    exit 1;
  }

  $plan_file = $cdr_outputfile;

  print "Converting domain $nodemap to CDR representation\n";
  $cdr_outputfile = $nodemap_base . ".cdr";
  $convert = $tg_convert_plan->CreateProcess("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_convert_plan",
                                               "-f " . $conversion_fmt . " -i " . $tg_convert_plan->LocalFile ($nodemap) .
                                               " -o " . $tg_convert_plan->LocalFile ($cdr_outputfile));
  ...
  if ($tg_convert_plan->WaitForFileTimed ($cdr_planbase, $tg_convert_plan->ProcessStartWaitInterval ()) == -1) {
    print STDERR "ERROR: dancex11_convert_plan failed to convert $nodemap to $cdr_outputfile.\n";
    kill_open_processes ();
    exit 1;
  }

  $nodemap = $cdr_outputfile;
}
...
print "Invoking domain deployment manager (dancex11_deployment_manager --handler dancex11_domain_dm_handler) with -l $plan_file and --nodemap $nodemap\n";
$EM = $tg_exe_man->CreateProcess ("$DANCEX11_ROOT/$DANCEX11_BIN_FOLDER/dancex11_deployment_manager",
                                  ...
                                  " -n ExecutionManager=$ior_emfile -- --nodemap " . $tg_exe_man->LocalFile($nodemap));
$em_status = $EM->Spawn ();
