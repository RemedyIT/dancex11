...
sub delete_ior_files {
    for ($i = 0; $i < $nr_daemon; ++$i) {
        ...
        $d_inst = $i+1;
        $tg_daemons[$i]->DeleteFile ("lm$d_inst.ior");
    }
...
sub run_node_daemons {
    for ($i = 0; $i < $nr_daemon; ++$i) {
        ...
        $d_inst = $i+1;
        $d_param = "--handler dancex11_node_dm_handler -a localhost -p $port -n $nodename=$iorfile " .
                   "-P \"nl.remedy.it.DnCX11.Locality.Arguments=\\\"-n LocalityInstance$d_inst=lm$d_inst.ior\\\"\" " .
                   "-P nl.remedy.it.DnCX11.DM.NumericAddresses=true";
